...
from userlist import  cathedralUsers
...
class PowerSchool(object):
    ...
    def procAttance(self, name, data):
        fn = self._genFileName(("powerschool",), (name, "attance"), suffix="html")
        with open(fn, "a+") as f:
            f.write(data)
            f.write("<!-- === -->\n")
        ...
        for tr in trs[1:]:
            ...
            for i,f in  enumerate(fields):
                ...
                items.append(item)
        fn = self._genFileName(("powerschool",), (name, "attance", "result"))
        with open(fn, "a+") as f:
            json.dump(items, f)
            f.write("\n")
        return items
    ...
    def procHistory(self, name, data):
        ...
        histroytime = "default"
        try:
            histroytime = soup.select('.tabs li a')[0].text.strip()
        except:
            pass
        items = []
        item = {}
        tdinfos = []
        curexp = "default"
        sel = soup.select(".box-round .grid tr")
        for tr in sel:
            ths = tr.select("th")
            if len(ths) == 1:
                if item:
                    item["sourseinfo"] = tdinfos
                    items.append(item)
                    item = {}
                    tdinfos = []
                curexp = ths[0].text.strip()
                item["exp"] = curexp
                continue
            ...
            tds = tr.select("td")
            if len(tds) != 5:
                print "td not eq 5", tr
                continue

            tdinfo = {}
            keys = ["course", "grade", "percent", "cit", "hrs"]
            for i, td in enumerate(tds):
                tdinfo[keys[i]] = td.text
            tdinfos.append(tdinfo)

        if item:
            item["courseinfo"] = tdinfos
            items.append(item)
        ret = {"time":histroytime, "data": items}
    ...
    def procAttanceHome(self, name, data):
        ...
        try:
            ths = sel[1].select("th")
            term1 = ths[4].text.strip()
            term2 = ths[5].text.strip()
        except:
            pass
        for tr in sel[3:-1]:
            scoresum = {}
            ...
            if ret:
                ...
                scoresum["teacherName"] = ret.group(4).replace(", ,", "")
                #print ret.groups()
            else:
            ...
            if ret:
                ...
                if ret:
                    ...
                    scoresum["score"] = ret.group(3)
                    scoresum["term"] = term2
            scoresum["absences"] = tds[14].text
            scoresum["tardies"] = tds[15].text
        ...
        if absa:
            try:
                absenceinfo["absencesSumUrl"] = self.baseurl + absa[0]["href"]
        ...
        if absa:
            try:
                absenceinfo["tardiesSumUrl"] = self.baseurl + absa[0]["href"]
            except Exception as e:
    ...
    def procScoreDetail(self, name, courseid, coursename, data):
        ...
        try:
            ...
            ret["teacherName"] = tds[1].text.replace(", ,", "")
            ret["exp"] = tds[2].text
        except Exception as e:
        ...
        trs  = tbs[1].select("tr")
        drs = []
        keys = ["dueDate", "category", "assignment", "score", "percent", "grd"]
        for tr in trs[1:-1]:
            td = tr.select("td")
            rs = {}
            rs[keys[0]] = td[0].text
    ...
    def test(self):
        ...
        #print json.dumps(ret)
        '''
        with open("history_cat.html", "r") as f:
            data = f.read()
        ret = self.procHistory("test", data)
        print ret
    ...
    def run(self, username, userpwd):
        driver = self.driver
        url = "https://cathedral.powerschool.com/public/home.html"
        #url = "https://cathedral.powerschool.com"

        driver.get(url)
        WebDriverWait(driver, 30).until(lambda the_driver: the_driver.find_element_by_xpath(
            "//input[@id='fieldAccount']").is_displayed())
        WebDriverWait(driver, 30).until(lambda the_driver: the_driver.find_element_by_xpath(
            "//input[@id='fieldPassword']").is_displayed())

        #print driver.page_source
        #time.sleep(0.5)

        name = driver.find_element_by_xpath("//input[@id='fieldAccount']")
        pwd = driver.find_element_by_xpath("//input[@id='fieldPassword']")
        submit = driver.find_element_by_xpath("//button[@id='btn-enter']")
        ...
        for ol in ret["scoreOutline"]:
            ...
            try:
                try:
                    ...
                    ela = eltd[13].find_element_by_css_selector("a")
            except Exception as e:
